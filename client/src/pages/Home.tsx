...
function RiskChart() {
  return <div className="line-chart"><div className="chart-grid"><span /><span /><span /><span /></div><svg className="line-svg" viewBox="0 0 620 155" preserveAspectRatio="none" aria-label="Tendência de risco"><defs><linearGradient id="riskFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#8C6AB0" stopOpacity=".26" /><stop offset="1" stopColor="#8C6AB0" stopOpacity="0" /></linearGradient></defs><path d="M0 112 C35 108, 43 72, 78 87 S115 108, 145 92 S173 62, 208 75 S244 96, 276 73 S315 61, 342 77 S380 93, 408 58 S442 35, 472 51 S505 62, 530 34 S573 40, 620 18 L620 155 L0 155 Z" fill="url(#riskFill)" /><path d="M0 112 C35 108, 43 72, 78 87 S115 108, 145 92 S173 62, 208 75 S244 96, 276 73 S315 61, 342 77 S380 93, 408 58 S442 35, 472 51 S505 62, 530 34 S573 40, 620 18" fill="none" stroke="#60397E" strokeWidth="2.5" vectorEffect="non-scaling-stroke" /><circle cx="472" cy="51" r="4" fill="#fff" stroke="#60397E" strokeWidth="2" vectorEffect="non-scaling-stroke" /></svg><div className="chart-tooltip">Hoje<strong>78</strong></div><div className="chart-labels"><span>03 set</span><span>04 set</span><span>05 set</span><span>06 set</span><span>07 set</span><span>08 set</span></div></div>;
}
...
function Dashboard({ search, onSelectRisk, onToast, onNavigate }: { search: string; onSelectRisk: (risk: Risk) => void; onToast: (message: string) => void; onNavigate: (view: ViewKey) => void }) {
  return <>
    ...
    <div className="panel" style={{ marginTop: 18, background: "#60397E", color: "#fff", borderColor: "#60397E" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}><div style={{ display: "flex", alignItems: "center", gap: 13 }}><div style={{ width: 34, height: 34, borderRadius: 9, display: "grid", placeItems: "center", background: "rgba(88,128,251,.22)", color: "#ffffff" }}><HardDriveUpload size={17} /></div><div><div style={{ fontSize: 12, fontWeight: 800 }}>Embarque EMB-3041 aguardando aceite</div><div style={{ color: "#D3C2E3", fontSize: 10, marginTop: 4 }}>VTR-2048 · 4 itens aceitos · 1 rejeitado (ponto inativo) · 1 credencial pendente</div></div></div><button className="soft-btn" onClick={() => { onToast("Abrindo provisionamento."); onNavigate("provisioning"); }}>Ver provisionamento <ChevronRight size={13} /></button></div></div>
  </>;
}

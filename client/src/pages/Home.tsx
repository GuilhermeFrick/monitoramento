...
function RealtimeView({ onSelectRisk, onToast }: { onSelectRisk: (risk: Risk) => void; onToast: (message: string) => void }) {
  ...
  return <>
    ...
    <div className="monitor-toolbar"><div className="monitor-live"><span className="live-pulse" /> AO VIVO <span>·</span> Atualizado há 8s</div><div className="monitor-toolbar-actions"><button className="secondary-btn" onClick={() => onToast("Filtros avançados disponíveis: frota, status, risco e câmera.")}><Filter size={13} /> Filtros</button><button className="secondary-btn" onClick={() => onToast("Layout salvo como estação do operador.")}><Settings size={13} /> Layout</button><button className="primary-btn" onClick={() => onToast("Patrulha iniciada com a seleção atual.")}><PlayCircle size={13} /> Iniciar patrulha</button></div></div>

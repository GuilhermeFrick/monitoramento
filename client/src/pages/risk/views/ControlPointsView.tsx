...
import { STORAGE, categoriaPontoLabel, newId, nivelDesvioLabel, nivelDesvioTone, nomePonto, nomePerfil, pontosIniciais, rotogramasIniciais, type CategoriaPonto, type NivelDesvio, type PerfilOperacional, type PontoDeControle, type Rotograma, type Trecho } from "../domain";
...
export function ControlPointsView({ pontos, setPontos, perfis, onToast }: { pontos: PontoDeControle[]; setPontos: (next: PontoDeControle[] | ((c: PontoDeControle[]) => PontoDeControle[])) => void; perfis: PerfilOperacional[]; onToast: (message: string) => void }) {
  ...
  return <>
    ...
    {tab === "pontos" ? <div className="panel" style={{ padding: 0 }}>
      ...
      <div className="table-wrap"><table><thead><tr><th>PONTO</th><th>CATEGORIA</th><th>POLÍTICA</th><th>PERFIL AO ENTRAR</th><th>PRECEDÊNCIA</th><th>FIXO</th><th>ATIVO</th></tr></thead><tbody>{pontos.map((p) => { const conflito = p.sobrepoe.length > 0 && p.precedencia === null; return <tr key={p.id}><td><div className="vehicle-cell"><div className="vehicle-avatar"><MapPin size={14} /></div><div><div className="vehicle-name">{p.nome}</div><div className="vehicle-meta">{p.id} · v{p.versao} · raio {p.raioM} m · {p.local}</div></div></div></td><td><Tag tone={p.categoria === "area_risco" ? "red" : p.categoria === "cliente" ? "teal" : "neutral"}>{categoriaPontoLabel[p.categoria]}</Tag></td><td style={{ fontSize: 10 }}>{p.politica.permanenciaMinMin ? `${p.politica.permanenciaMinMin}–` : "≤ "}{p.politica.permanenciaMaxMin} min · {p.politica.janela}</td><td style={{ fontSize: 10 }}>{nomePerfil(p.politica.perfil, perfis)}</td><td>{p.sobrepoe.length === 0 ? <span style={{ color: "#a3adb8", fontSize: 10 }}>—</span> : <select className="filter-select" value={p.precedencia ?? ""} onChange={(e) => { setPontos((c) => c.map((x) => x.id === p.id ? { ...x, precedencia: e.target.value ? Number(e.target.value) : null } : x)); onToast("Precedência atualizada."); }} style={conflito ? { borderColor: "#d1635c" } : undefined}><option value="">Sobrepõe {p.sobrepoe.join(", ")} · definir</option><option value={1}>1 · prevalece</option><option value={2}>2 · cede</option></select>}</td><td>{p.fixo ? <Tag tone="dark"><Pin size={10} /> fixo</Tag> : <button className="panel-link" onClick={() => { setPontos((c) => c.map((x) => x.id === p.id ? { ...x, fixo: true } : x)); onToast("Ponto marcado como fixo: sobrevive à limpeza da política embarcada."); }}>marcar fixo</button>}</td><td><Toggle checked={p.ativo} onChange={(next) => { setPontos((c) => c.map((x) => x.id === p.id ? { ...x, ativo: next } : x)); onToast(next ? "Ponto ativado." : "Ponto inativado. Embarques que o referenciam serão rejeitados pelo equipamento."); }} /></td></tr>; })}</tbody></table></div>
    </div> : null}
    ...
    {showNew ? <NewPointModal perfis={perfis} onClose={() => setShowNew(false)} onCreate={(ponto) => { setPontos((c) => [...c, ponto]); setShowNew(false); onToast(`Ponto de controle “${ponto.nome}” cadastrado (v1).`); }} /> : null}
  </>;
}

function NewPointModal({ perfis, onClose, onCreate }: { perfis: PerfilOperacional[]; onClose: () => void; onCreate: (ponto: PontoDeControle) => void }) {

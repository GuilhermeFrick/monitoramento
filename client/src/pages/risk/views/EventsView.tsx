...
import { STORAGE, canalLabel, eventosIniciais, newId, nomePerfil, nomePonto, type Comentario, type Evento, type PerfilOperacional, type PontoDeControle } from "../domain";
...
export function EventsView({ pontos, perfis, onToast, onCommand }: { pontos: PontoDeControle[]; perfis: PerfilOperacional[]; onToast: (message: string) => void; onCommand: (veiculo: string) => void }) {
  ...
  return <>
    ...
    {selected ? <EventDetail evento={selected} pontos={pontos} perfis={perfis} onClose={() => setSelectedId(null)} onAdd={(t) => addComentario(selected.id, t)} onEdit={(cid, t) => editComentario(selected.id, cid, t)} onCommand={() => { setSelectedId(null); onCommand(selected.veiculo); }} /> : null}
  </>;
}

function EventDetail({ evento, pontos, perfis, onClose, onAdd, onEdit, onCommand }: { evento: Evento; pontos: PontoDeControle[]; perfis: PerfilOperacional[]; onClose: () => void; onAdd: (t: string) => void; onEdit: (cid: string, t: string) => void; onCommand: () => void }) {

...
export function ProvisioningView({ pontos, onToast }: { pontos: PontoDeControle[]; onToast: (message: string) => void }) {
  ...
  const emitir = (veiculo: string, itens: CatalogoItem[]) => { const eq = equipamentos.find((e) => e.veiculo === veiculo)!; const novo: Embarque = { id: newId("EMB"), veiculo, equipamento: eq.equipamento, emitidoEm: new Date().toISOString(), emitidoPor: "User Teste", itens: itens.map((i) => ({ id: newId("i"), tipo: i.tipo, referencia: i.id, nome: i.nome, versao: i.versao, aceite: "pendente" })) }; setEmbarques((c) => [novo, ...c]); setSelectedId(novo.id); setShowEmit(false); onToast(`Embarque ${novo.id} emitido para ${eq.equipamento} com ${itens.length} item(ns). Aguardando aceite individual.`); };
  const limpar = (veiculo: string) => { const eq = equipamentos.find((e) => e.veiculo === veiculo)!; const fixos = pontos.filter((p) => p.fixo); const novo: Embarque = { id: newId("EMB"), veiculo, equipamento: eq.equipamento, emitidoEm: new Date().toISOString(), emitidoPor: "User Teste", itens: [{ id: newId("i"), tipo: "jornada", referencia: "LIMPEZA", nome: "Limpeza da política embarcada (jornada, pontos, cercas, perfis)", versao: 0, aceite: "pendente" }, ...fixos.map((p) => ({ id: newId("i"), tipo: "ponto" as const, referencia: p.id, nome: `${p.nome} (fixo · mantido)`, versao: p.versao, aceite: "aceito" as const, respondidoEm: new Date().toISOString() }))] }; setEmbarques((c) => [novo, ...c]); setSelectedId(novo.id); setShowClean(false); onToast(`Limpeza emitida para ${eq.equipamento}. ${fixos.length} ponto(s) fixo(s) preservado(s).`); };
